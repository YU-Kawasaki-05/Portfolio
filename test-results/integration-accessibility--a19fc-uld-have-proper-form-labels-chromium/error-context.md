# Page snapshot

```yaml
- banner:
  - navigation:
    - link "K KawasakiK":
      - /url: /
    - link "Home":
      - /url: /
    - link "Works":
      - /url: /portfolio
    - link "Blog":
      - /url: /blog
    - link "Profile":
      - /url: /profile
    - link "Services":
      - /url: /services
    - link "SNS":
      - /url: /sns
- main:
  - paragraph: Loading...
  - heading "Works" [level=1]
  - paragraph: 実績とプロジェクトの詳細をご紹介します
  - textbox "プロジェクトを検索..."
  - combobox:
    - option "すべてのタグ" [selected]
    - option "Animation"
    - option "CSS"
    - option "Framer Motion"
    - option "GLSL"
    - option "JavaScript"
    - option "Next.js"
    - option "React"
    - option "Three.js"
    - option "TypeScript"
    - option "WebGL"
  - paragraph: 3件のプロジェクトが見つかりました
  - table:
    - rowgroup:
      - row "プロジェクト名 日付 タグ アクション":
        - cell "プロジェクト名"
        - cell "日付"
        - cell "タグ"
        - cell "アクション"
    - rowgroup:
      - row "Neo-Typographic Fusion Portfolio 3Dタイポグラフィとモダンデザインを融合させたポートフォリオサイト 2025/07/14 React Three.js +2 詳細を見る":
        - cell "Neo-Typographic Fusion Portfolio 3Dタイポグラフィとモダンデザインを融合させたポートフォリオサイト":
          - heading "Neo-Typographic Fusion Portfolio" [level=3]
          - paragraph: 3Dタイポグラフィとモダンデザインを融合させたポートフォリオサイト
        - cell "2025/07/14"
        - cell "React Three.js +2"
        - cell "詳細を見る":
          - link "詳細を見る":
            - /url: /portfolio/neo-fusion-portfolio
      - row "Interactive 3D Gallery WebGLを使用したインタラクティブな3Dギャラリーアプリケーション 2023/11/20 Three.js WebGL +2 詳細を見る":
        - cell "Interactive 3D Gallery WebGLを使用したインタラクティブな3Dギャラリーアプリケーション":
          - heading "Interactive 3D Gallery" [level=3]
          - paragraph: WebGLを使用したインタラクティブな3Dギャラリーアプリケーション
        - cell "2023/11/20"
        - cell "Three.js WebGL +2"
        - cell "詳細を見る":
          - link "詳細を見る":
            - /url: /portfolio/interactive-3d-gallery
      - row "Motion Design Website Framer Motionを活用したアニメーション豊富なWebサイト 2023/09/10 Framer Motion React +2 詳細を見る":
        - cell "Motion Design Website Framer Motionを活用したアニメーション豊富なWebサイト":
          - heading "Motion Design Website" [level=3]
          - paragraph: Framer Motionを活用したアニメーション豊富なWebサイト
        - cell "2023/09/10"
        - cell "Framer Motion React +2"
        - cell "詳細を見る":
          - link "詳細を見る":
            - /url: /portfolio/motion-design-website
- contentinfo:
  - text: K
  - heading "KawasakiK" [level=3]
  - paragraph: 広範な生成AI知識と開発知見を活用し開発した川崎Kの個人サイト。 React、Next.js、Three.jsを使用したWebエクスペリエンス。
  - link "GitHub":
    - /url: https://github.com/YU-Kawasaki-05
  - link "X":
    - /url: https://x.com/foooten_
  - link "LinkedIn":
    - /url: https://www.linkedin.com/in/yu-kawasaki-a05441296/
  - link "Email":
    - /url: mailto:contact@neo-fusion.dev
  - heading "Navigation" [level=4]
  - list:
    - listitem:
      - link "Home ホーム":
        - /url: /
    - listitem:
      - link "Works 事例と実績":
        - /url: /portfolio
    - listitem:
      - link "Blog ブログ":
        - /url: /blog
    - listitem:
      - link "Profile プロフィール":
        - /url: /profile
  - heading "Services" [level=4]
  - list:
    - listitem:
      - link "Services サービス":
        - /url: /services
    - listitem:
      - link "SNS ソーシャル":
        - /url: /sns
  - heading "Resources" [level=4]
  - list:
    - listitem:
      - link "Tech Blog 技術ブログ":
        - /url: /blog
    - listitem:
      - link "Consulting コンサルティング":
        - /url: /services
  - paragraph: © 2025 KawasakiK. All rights reserved.
  - link "Privacy Policy":
    - /url: /privacy
  - link "Terms of Service":
    - /url: /terms
  - text: Made with ❤️ and ☕ in Tokyo
- button "モーションを無効化":
  - img
- alert
- button "Open Next.js Dev Tools":
  - img
```